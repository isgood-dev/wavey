...
class MainWindow(Tk):
    ...
    def play(self, audiofile=None, enable_loop=False):
        ...
        self.sc._play(file=str(os.getcwd()) + f"/Audio bin/{audiofile}.mp3")
    ...
    def stop(self):
        ...
        self.sc._stop() # We pause so that we don't release resources
                        # which makes the player so to respond when
                        # another song is added.

    def pause_or_resume(self, _=None):
        if not self.sc.song:
